...
const TRANSLATION_KEY_PREFIX = APIErrorKeys.organization.cancelInvitation;
...
export const CancelInvitationValidator: RequestHandler = async (
  req,
  res,
  next,
) => {
  try {
    const body = cancelInvitationSchema.parse(req.body);
    req.body = body;
